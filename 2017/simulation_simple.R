...
# maternal height - gestational age study
# very simple way of simulationg competing birth-risks (baseline and interaction)
# 2017-10-02
...
# generate variables
gas = rgompertz(n_ind*1.1,0.1212,1.026*1e-8) + 150 # natural gest.age-at-birth distribution
...
### determine the actual Gestational age
...
fun_run(0,F,F,F,T,F) # base_shift,densit,scaPlot,graHistA,graHistB,graHistX
...
####################
### counterfactual: what if MUS and FGR manifest effect independently
####################
...
dat$cause = NA
dat$cause[which(dat$GAintrx < dat$GAbase)] = "X"
dat$cause[which(dat$GAbase < dat$GAintrx)] = "B"
dat$cause[which(dat$GAintrx == dat$GAbase)] = "N"
table(dat$cause)
