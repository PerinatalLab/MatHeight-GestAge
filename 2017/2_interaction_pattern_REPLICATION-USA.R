...
dat1 = dat[which((dat$PRIORLIVE==0)&(dat$PRIORDEAD==0)&(dat$PRIORTERM==0)),]
dim(dat1)
...
sin = sin[which((!is.na(sin$DBWT))&(!is.na(sin$GRDBS))),]  
...
sin$KON = NA
sin$KON[which(sin$SEX=="M")] = 1
sin$KON[which(sin$SEX=="F")] = 2


# estimate BWzsc (pretending that sex is unknown for all children)
sin$bw_zsc_Marsal=sapply(1:nrow(sin), function(x) marsal(sin$GRDBS[x],sin$DBWT[x],sex=sin$KON[x]))
head(sin); dim(sin)
...
# some corrections
sin$M_Ht_In[which(sin$M_Ht_In>75)] = 75

######  polynomial linear regression model
mod = lm(GRDBS ~ poly(M_Ht_In,2) * cat, data = sin)
mod = lm(GRDBS ~ M_Ht_In * factor(cat,levels=c("SGA","LGA")), data = sin)  # height in inches
summary(mod)
#table(sin$M_Ht_In)
...
fun = function() {
        ...
        df = expand.grid(cat=c("LGA","SGA"),M_Ht_In=56:75)
        df = df[order(df$cat,df$M_Ht_In),]
        prd = predict(mod,df,interval = "confidence",level = 0.95) # 
        df$nw = prd[,1] # new value (predicted one)
        tst = df$cat=="SGA"
        plot(df$M_Ht_In[tst],df$nw[tst],col="cornflowerblue",type="l",
             ylim=c(min(df$nw)*0.98,max(df$nw)*1.01),
             ylab="child's gestational age at birth (days)",xlab="maternal height (inch)")
        grid()
        points(df$M_Ht_In[!tst],df$nw[!tst],col="orange",type="l")
        #clean the area for legends (white background)
        #polygon(x = c(151,191,191,151,151),y=c(265,265,272,272,265),col = "white",border = NA) 
        
        ##### confidence intervals for the model curves
        # for SGA
        xs = c(df$M_Ht_In[tst],rev(df$M_Ht_In[tst]),df$M_Ht_In[tst][1])
        ys = c(prd[tst,2],rev(prd[tst,3]),prd[tst,2][1])
        clrcm = col2rgb("cornflowerblue") / 255 # color components
        ...
        # LEGEND 1
        x = 61; dx = 2; xs = c(x,x+dx,x+dx,x,x)
        y = 259; dy = 0.5; ys = c(y-dy,y-dy,y+dy,y+dy,y-dy)
        polygon(xs,ys,col=ciclr_sga,border = NA)
        segments(x,y,x+dx,y,col="cornflowerblue") # SGA
        polygon(xs,ys-1.3,col=ciclr_lga,border = NA)
        segments(x,y-1.3,x+dx,y-1.3,col="orange")
        text(x-2.3,y+1.5,"model, 95% CI",pos = 4,cex = 0.7)
        text(x+1.5,y,"SGA",pos = 4,cex = 0.7)
        text(x+1.5,y-1.3,"LGA",pos = 4,cex = 0.7)
        
        ###### data for empirical means
        df_sing = group_by(sin,cat,M_Ht_In) %>% 
                summarise(n=n(),mnGA=mean(GRDBS),seGA=sd(GRDBS)/sqrt(n)) %>% filter(n>10) %>% ungroup() 
        ...
        vls = df_sing$M_Ht_In[which(df_sing$cat=="LGA")]
        df_sing$M_Ht_In[which(df_sing$cat=="LGA")] = vls - 0.2
        
        # data points
        points(y = df_sing$mnGA, x = df_sing$M_Ht_In,pch=19,col=df_sing$clr, cex=log(df_sing$n,50))
        points(y = df_sing$mnGA, x = df_sing$M_Ht_In,cex=log(df_sing$n,50)) # "coat"
        # 95% Conf.intervals
        df_sing$upp = df_sing$mnGA + 1.96*df_sing$seGA
        df_sing$low = df_sing$mnGA - 1.96*df_sing$seGA
        segments(x0 = df_sing$M_Ht_In,x1 = df_sing$M_Ht_In,
        ...
        # LEGEND 2
        x = 66; y = 259; dy = 0.5
        text(x-2,y+1.5,"mean, 95% CI",pos = 4,cex = 0.7)
        ...
        # LEGEND 3
        x = 68.5; y = 261.5
        text(x-1,y,"observations (n)",pos = 4,cex = 0.7)
        szs = c(1e2,1e3,1e4)
        legend(x,y,legend = szs,pch=1,bty = "n",pt.cex = log(szs,50))
