...
mod0 = lm(dat$GRDBS[which(dat$status=="singl")] ~ dat$MLANGD[which(dat$status=="singl")])
summary(mod0) # 0.133 beta
coef(summary(mod0))
mod1 = lm(dat$GRDBS[which(dat$status=="twins")] ~ dat$MLANGD[which(dat$status=="twins")])
summary(mod1) # 0.413 beta
coef(summary(mod1))
#

##### linear interaction model
mod2 = lm(dat$GRDBS ~ dat$MLANGD * dat$status)
summary(mod2)
coef(summary(mod2))
cfs = coef(summary(mod2)); cfs
...
polygon(xs,ys-5,col=ciclr_lga,border = NA)
...
mod0 = lm(sin$GRDBS[sin$cat=="SGA"] ~ sin$MLANGD[sin$cat=="SGA"])
summary(mod0) # 0.100 beta
mod1 = lm(sin$GRDBS[sin$cat=="LGA"] ~ sin$MLANGD[sin$cat=="LGA"])
summary(mod1) # 0.197 beta

##### model with linear interaction 
sin$cat = factor(sin$cat,levels = c("SGA","LGA"))
mod2 = lm(GRDBS ~ MLANGD*cat,data=sin)
summary(mod2) 
coef(summary(mod2))
